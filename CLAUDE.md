...
| Penalty Box | `Home > [Event Name] > [Penalty Box Name]` |
...
- `league/[slug]/` — League info page (leaderboard, scoring, rules). Only accessible for events with league_enabled=true. Contains: `page.tsx` (server component), `league-tabs.tsx` (tab switching with Points/Money toggle), `leaderboard.tsx` (sortable points standings grid with sticky columns), `money-leaderboard.tsx` (sortable money standings grid with $ formatting).
- `penalty-box/[slug]/` — Penalty Box page (current inmates, history log). Accessible for events with `penalty_box_enabled=true`. Contains: `page.tsx` (server component), `penalty-box-client.tsx` (interactive inmate list + history), `actions.ts` (server actions for escape, witness selection, apology submission).
- `penalty-box/[slug]/escape/[penaltyId]/` — 3-hole mini golf escape game (penalized golfer only). Contains: `page.tsx`, `escape-game-client.tsx` (power meter mechanic, hole progression, clown reveal on hole 3).
- `penalty-box/[slug]/witness/[token]/` — Character witness flow (tokenized, no login required). Contains: `page.tsx`, `witness-flow-client.tsx` (mini golf game → yes/no vote + required comment).
...
- `golfers/[golferId]/actions.ts` — Global golfer detail server actions
...
- `events/[eventId]/golfers/[golferId]/actions.ts` — Event-scoped golfer detail server actions
...
- `events/[eventId]/penalty-box-admin.tsx` — Admin Penalty Box panel (send golfer to Penalty Box with charge, view active penalties, release, manage)
...
- `schedule-gen.ts` — Auto-generate weekly schedule rows for an event. `calculateEventEndDate()` returns the season's last game date (fixed_weeks = `start_date + (duration_weeks - 1) * 7`; end_date mode = stored `end_date`; indefinite = null).
- `schedule-gen.test.ts` — Unit tests for schedule date math (`calculateEventEndDate` off-by-one regression, game-date generation). Run with `npx tsx --test src/lib/schedule-gen.test.ts`.
...
- `grouping-db.ts` — DB queries: fetch confirmed golfers (with handicap resolution), partner preferences, approved guests, restricted (do-not-pair) pairs; store groupings with guest placement and team numbers; fetch stored groupings with tee time + partner preference annotations
- `penalty-box.ts` — Penalty Box business logic (fetch active/historical penalties by event slug, create penalty, record escape, manage witnesses, submit apology, release golfer, check/expire timed-out witnesses)
- `penalty-box-emails.ts` — Penalty Box email templates and send helpers (penalty issued, witness request, witness no-vote, witness timeout, apology notification, release announcement)
...
- `league.ts` — League info helpers: fetch config/tabs/scores/money-scores by slug or event ID, compute season weeks, build leaderboard with best-N-of-M aggregation and rank calculation, build money leaderboard with total dollar rankings
...
- `scripts/import-scores.ts` — Import weekly Thursday League Stableford scores from a Golf Genius Excel export
...
- Feature flags (guest requests, Penalty Box with configurable display name). Note: tee time preferences and playing partner preferences are now managed in the Grouping Engine section (super admin only), not Feature Flags.
...
- Restricted Pairings (do-not-pair): Admin-only list of golfer pairs who must never be grouped together. Managed in the Grouping Engine section of Event Settings. Applied as the highest-priority constraint by the grouping engine across all grouping methods.
...
### Season Boundaries (Start & End Date Guards)
Events have a `duration_mode` of `indefinite`, `fixed_weeks`, or `end_date`. For fixed-length seasons the last game is `start_date + (duration_weeks - 1) * 7` — `start_date` is the **first** game, so a 10-week season starting May 7 ends on the 10th game (July 9), NOT `start + 10*7` (July 16). This is computed by `calculateEventEndDate()` in `schedule-gen.ts` and stored into `events.end_date` by the create/settings actions.

The `email-scheduler` cron enforces both boundaries per run: it skips an event when the upcoming game date is **before** `start_date` OR **after** `calculateEventEndDate(event)`. Without the end guard, an active event with enabled email schedules would generate a fresh schedule row and send an invite every week forever, because the scheduler computes "next game day" generically and `ensureSchedule()` creates the row on demand. Indefinite events return a null end date and are never bounded. (Regression coverage in `schedule-gen.test.ts`.)
...
- Phone (optional, US 10-digit format when provided)
...
Low H.I. (12-month low handicap index) (migration 028):
- `profiles.low_hi_value` — numeric(4,1), 12-month low Handicap Index from GHIN. NULL = never synced or no data available. Sentinel value 999 from GHIN API means "no data" and is stored as NULL.
- `profiles.low_hi_date` — date, the date the low H.I. pertains to. NULL = no data available.
- Both fields are populated by the existing GHIN sync process (no additional API calls). Displayed alongside current handicap index on: golfer profile, admin golfer detail pages, golfer directories, suggested groupings email, and the league leaderboard.
...
- `profiles.manual_handicap_index` — numeric(4,1), legacy column (no longer used in the app). The column remains in the database but is not read or written by any code.
...
Money leaderboard (migration 030):
- `league_money_scores` table: weekly dollar winnings per golfer per game date. Columns: `event_id`, `profile_id`, `game_date`, `amount` numeric(8,2), `metadata` (JSONB), `entered_by`. Unique constraint on `(event_id, profile_id, game_date)`. RLS: authenticated read, super admin manage, service role full. Indexed on `(event_id, game_date)` and `(profile_id)`.
...
Penalty Box (migration 032):
- `events.penalty_box_enabled` — boolean, default false. Feature flag to enable/disable Penalty Box per event.
- `penalty_box` table: one row per penalty instance. Columns: `id`, `event_id`, `profile_id` (penalized golfer), `charged_by`, `charge` (text), `status` (CHECK: `incarcerated`, `awaiting_witnesses`, `apology_required`, `released`), `escape_completed_at`, `released_at`, `released_by`, `apology_text`, `apology_submitted_at`, `witnesses_required` (smallint, default 3), `offense_number` (auto-calculated, default 1), `total_no_votes` (smallint, default 0), `created_at`. RLS: authenticated users can read all (public visibility); admins can insert and update; penalized golfer can update own penalty; service role full access.
- `penalty_witnesses` table: one row per witness request. Columns: `id`, `penalty_id` (FK), `witness_profile_id` (FK), `token` (uuid, unique — for tokenized witness links), `status` (CHECK: `pending`, `completed`, `expired`), `vote` (CHECK: `yes`, `no`), `testimony`, `game_completed_at`, `voted_at`, `expires_at`, `created_at`. UNIQUE constraint on `(penalty_id, witness_profile_id)` — same witness cannot be asked twice per penalty. RLS: authenticated users can read all; admins can insert and update; penalized golfer can insert witnesses; witnesses can update own record; service role full access.
- `email_log` CHECK constraint updated to include 6 new penalty email types: `penalty_issued`, `penalty_witness_request`, `penalty_witness_no`, `penalty_witness_timeout`, `penalty_apology`, `penalty_released`.

Configurable Penalty Box name (migration 033):
- `events.penalty_box_name` — text, NOT NULL, DEFAULT `'The Penalty Box'`. Per-event custom display name for the Penalty Box feature.
...
- **Async work in Server Actions MUST be awaited.** Vercel terminates a serverless function as soon as the Server Action returns. Any async operation (email sending, DB writes) that is not `await`-ed will be killed before it completes. Do NOT use fire-and-forget patterns (`.catch()`-only callbacks, `void asyncFn()`) for work that must actually happen. Use `await` wrapped in `try/catch` so a failure doesn't roll back the already-committed operation.
...
The `email_log.email_type` column has a CHECK constraint allowing only: `'invite'`, `'reminder'`, `'confirmation_golfer'`, `'confirmation_proshop'`, `'no_game'`, `'guest_approved'`, `'guest_denied'`, `'guest_request_pending'`, `'registration_pending'`, `'custom'`, `'penalty_issued'`, `'penalty_witness_request'`, `'penalty_witness_no'`, `'penalty_witness_timeout'`, `'penalty_apology'`, `'penalty_released'`. Do NOT use suffixed types like `'reminder_1'` or `'reminder_manual'` — they will silently fail the constraint and the log entry won't be written.
...
- **Admin Tools:** Schedule management (8-week rolling view, Game On/No Game toggle with cancellation emails [awaited properly in serverless — not fire-and-forget]), custom email composer with templates, action items/task summary, golfer directory with search/filter (global + event-scoped), golfer detail pages with subscription management, admin "Add Golfer" (direct add), admin "Add Golfer to Game" on RSVP management page (add subscribed golfers who missed the invite cycle), configurable email schedules (6 Vercel cron slots), admin notification emails (new_registration, capacity_reached, spot_opened, low_response), event-centric admin dashboard with summary cards, per-event admin scoping, help page with Golfer + Admin FAQ. Next-game display respects event `start_date` — events with a future start date don't show games before that date.
- **Grouping Engine:** Five grouping methods: (1) Harmony — greedy heuristic with weighted partner preferences, tee time constraints, shuffle randomization; (2) Flight Foursomes — sorted by handicap, grouped by skill level; (3) Balanced ABCD Foursomes — round-robin tier distribution (one from each quartile per group); (4) Flight 2-Person Teams — adjacent handicap pairs, with similar or random foursome pairing; (5) Balanced 2-Person Teams — outside-in pairing (best+worst) for equal team totals. Handicap methods override partner/tee time preferences (with admin warning banner). Handicap resolution: handicap_index → 25.0 default. Guest-host pairing, group variety with 8-week lookback, configurable partner/tee time modes. **Restricted Pairings (Do-Not-Pair):** Admin-only feature to designate golfer pairs who must never be grouped together. Applied as the highest-priority constraint in all grouping methods: `-99999` score penalty in `applyScoreModifiers` (harmony method) plus a universal `separateRestrictedPairs` post-processing pass that swaps violating golfers across groups. UUIDs stored in order-normalized pairs to enforce uniqueness. Managed via `RestrictedPairingsSection` in Event Settings (Grouping Engine section). Migration 034. 50+ unit tests. DB layer with team_number support, cron integration, suggested groupings email with grouped roster. See `docs/GROUPING_ENGINE_SPEC.md`.
...
- **GHIN Handicap Sync:** Automated system to fetch current Handicap Index and 12-month Low H.I. from GHIN for all golfers with a GHIN number. Uses unofficial GHIN mobile app API (`api2.ghin.com`) directly via HTTP — no npm dependency, fully patchable. Auth uses `email_or_ghin` + `password` + `token` fields. Syncs within 24 hours of each scheduled event via email-scheduler cron. Features: per-event toggle (`handicap_sync_enabled`), 20-golfer batch cap per cron run (stalest-first), 24-hour freshness window (shared across events for multi-event golfers), `handicap_sync_log` table for health monitoring, auto-alert on auth/total failure, status indicator in Event Settings. Handicap and Low H.I. displayed on: golfer home page (My Profile section), golfer profile/edit page, admin golfer detail pages (global + event-scoped), both golfer directories (global + event-scoped), suggested groupings email (HCP + Low H.I. columns), and league leaderboard (Low H.I. column). Env vars: `GHIN_EMAIL`, `GHIN_PASSWORD` (must be set at the **Vercel project level**, not team level). Handicap history: every GHIN-synced value is recorded in the `handicap_history` table for per-golfer and group-level trend analysis. See `docs/HANDICAP_SYNC_SPEC.md`.
...
- **League Info & Leaderboard (Phase 1):** Per-event league info system with configurable tabs (Leaderboard, Scoring & Prizes, Conditions of Play). Three new tables: `event_league_config` (season settings, best-N-of-M scoring, prize payout config), `event_league_tabs` (flexible tab definitions with HTML or data-driven content types), `league_scores` (weekly Stableford points per golfer). Golfer-facing page at `/league/[slug]` with sortable leaderboard grid (two-row header with week numbers and dates, sticky Rank/Golfer/Total columns for mobile horizontal scroll, counting vs. dropped score visual treatment, DNP for missed weeks, tie-shared ranks). "League Info" link conditionally appears in the event RSVP card on the golfer home page only for events with `league_enabled=true`. Seeded for Thursday League with full HTML content for Scoring and Rules tabs. Score entry method TBD pending first Golf Genius report. Migration 024. **Season-prize qualification status:** the points leaderboard shows a per-golfer badge next to their name reflecting `min_rounds_to_qualify` — "Qualified" (teal, rounds played already meets the minimum), "Must Play [date]" (amber, below the minimum but can still reach it in a remaining unplayed week), or "Not Qualified" (gray, mathematically eliminated — not enough remaining weeks left). Remaining weeks are computed via `getRemainingWeeksInfo()` in `src/lib/league.ts`, which treats a season week as "unplayed" until any golfer has a recorded score for that date (not by comparing to today's date, so it's unaffected by import timing). Weekly prizes are unaffected — this only governs season-long prize eligibility. See `docs/LEAGUE_INFO_SPEC.md`.
- **Money Leaderboard:** Tracks weekly dollar winnings per golfer alongside the existing Stableford points leaderboard. New `league_money_scores` table (migration 030) stores total dollars won per golfer per week. The Leaderboard tab now has a Points/Money toggle — same grid layout with Rank, Golfer, weekly columns, and Total, but the Money view shows `$` amounts with green highlighting for winnings, `$0` for played-but-no-winnings, and `DNP` for missed weeks. All weekly dollars count toward the total (no best-N-of-M). Data entry workflow: admin pastes weekly winner email results into a Cowork chat, Claude parses and confirms totals, then inserts via SQL. New files: `money-leaderboard.tsx` (component), types `LeagueMoneyScore` and `MoneyLeaderboardEntry`, library functions `getLeagueMoneyScores()` and `buildMoneyLeaderboard()`.
...
- **Penalty Box:** Gamified penalty system (per-event, feature-flagged). Admins can "penalize" a golfer with a written charge — all subscribers are notified by email. The penalized golfer completes a 3-hole power meter mini golf challenge: holes 1–2 are real (easy/medium), hole 3 always spits the ball back out (the clown, named after the event admin). After hole 3, the golfer selects 3 character witnesses from the subscriber list. Each witness plays the same game then votes yes/no with a required comment. Yes votes progress toward release; no votes require a replacement witness. Three total no votes triggers an apology flow to the admin. Release sends a group-wide email with time served and witness testimonies. Admin can release unilaterally at any time. Repeat offenders require more witnesses (1st: 3, 2nd: 5, 3rd+: 7). Penalty Box name is configurable per event (default "The Penalty Box"). Pages: `/penalty-box/[slug]` (public page), `/penalty-box/[slug]/escape/[penaltyId]` (putting game), `/penalty-box/[slug]/witness/[token]` (tokenized witness flow). Migrations 032–033. See `docs/PENALTY_BOX_SPEC.md`.
